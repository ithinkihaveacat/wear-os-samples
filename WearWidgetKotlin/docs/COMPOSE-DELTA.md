# Remote Compose vs. Standard Compose Deltas
...
and rendering happen in another (the System UI).
...
**Why:** To minimize IPC traffic and battery usage, the system avoids waking
your app for every frame. Logic inside `RemoteCanvas` runs only at composition
time. Dynamic behavior requires `drawConditionally`.
...
a dependency graph in the document. `count` is assigned a state ID, and the string
concatenation becomes a "Merge" instruction dependent on that ID. When the
state changes on the host (e.g., via a click action), the renderer re-evaluates
the merge and repaints the text—all while your app process remains asleep.
...
**Why:** The conditional logic must be encoded into the protocol so the System
UI can switch values locally when the state changes.
...
**Why:** The renderer handles the interpolation locally. The app process simply
declares "animate changes" and sends the new end-state, minimizing IPC.
...
update the UI, but the app's composition logic is *not* re-executed.

**Why:** This allows the app to provide a blueprint and then suspend. The host
manages the UI lifecycle and state updates locally, ensuring high performance
without the cost of waking the app process for every frame or interaction.
...
## Likely Temporary Limitations (Bugs or Omissions)
...
## Unsure / API Design Choices

Differences where the "correct" long-term behavior is ambiguous or involves
complex trade-offs.
