...
private fun searchLayout(context: Context, clickable: ModifiersBuilders.Clickable) =
    Button.Builder(context, clickable)
...
private fun messagingTilePreview(context: Context): TilePreviewData {
    ...
    return TilePreviewData(
        ...
        resources {
            addIdToImageMapping(state.contacts[1].imageResourceId(), R.drawable.ali)
            addIdToImageMapping(state.contacts[2].imageResourceId(), R.drawable.taylor)
            addIdToImageMapping(MessagingTileService.ID_IC_SEARCH, R.drawable.ic_search_24)
        },
        onTileRequest = { request ->
            TilePreviewHelper.singleTimelineEntryTileBuilder(
                messagingTileLayout(state, context, request.deviceConfiguration)
            )
                .build()
        }
    )
...
@Preview
private fun contactPreview(context: Context) =
    TilePreviewData(
        onTileRequest = {
            TilePreviewHelper.singleTimelineEntryTileBuilder(
                contactLayout(
...
@Preview
private fun searchButtonPreview(context: Context) =
    TilePreviewData(
        onTileResourceRequest = {
            Resources.Builder()
                .addIdToImageMapping(MessagingTileService.ID_IC_SEARCH, R.drawable.ic_search_24)
        ...
        onTileRequest = {
            TilePreviewHelper.singleTimelineEntryTileBuilder(searchLayout(context, emptyClickable))
                .build()
        }
    )
